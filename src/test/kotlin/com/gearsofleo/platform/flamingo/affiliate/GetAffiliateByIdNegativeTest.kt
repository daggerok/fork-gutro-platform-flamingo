...
@DisplayName("Flamingo REST API negative test of get affiliate by id operation")
class GetAffiliateByIdNegativeTest @Autowired constructor(
    ...
    @Test
    fun `should not get affiliate by id which is not in db`() {
        // @formatter:off

        // setup
        given(affiliateClient.getAffiliateById(any())) willThrow {
            AffiliateNotFoundException("with-not-found-id")
        }

        Given {
            spec(of("get-affiliate-by-id-negative-not-found"))

            contentType(MediaType.APPLICATION_JSON_VALUE)
        }.

        When {
            get(url("/api/v1/affiliate/with-not-found-id"))
        }.
